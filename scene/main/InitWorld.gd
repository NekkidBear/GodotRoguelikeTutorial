...
func _unhandled_input(event: InputEvent) -> void:
    if event.is_action_just_pressed(_new_InputName.INIT_WORLD):
        _init_floor()
        _init_wall()
        _init_PC()
        _init_dwarf()
...
func _init_indicator() -> void:
    _create_sprite(ArrowX, _new_GroupName.ARROW, 0, 12, -_new_DungeonSize.ARROW_MARGIN)
    _create_sprite(ArrowY, _new_GroupName.ARROW, 5, 0, 0, -_new_DungeonSize.ARROW_MARGIN)
